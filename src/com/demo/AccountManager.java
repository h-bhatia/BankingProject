...
public void debit_money(long account_number)throws SQLException
{
	...
	if(resultSet.next())
	{
		...
		if(amount<=current_balance)
		{
			...
			if(rowsAffected>0)
			{
				System.out.println("Rs." +amount+ "debited successfuly");
				connection.commit();
...
public void credit_money(long account_number)throws  SQLException
{
	...
	try {
		...
		if(resultSet.next())
		{
			...
			PreparedStatement pst1=connection.prepareStatement(credit_query);
			pst1.setDouble(1, amount);
			pst1.setLong(2, account_number);
			int rowsAffected=pst1.executeUpdate();
			if(rowsAffected>0)
...
public void getBalance(long account_number)
{
	...
	String security_pin=scanner.nextLine();
	try {
		PreparedStatement pst=connection.prepareStatement("select balance from Accounts where account_number=? and security_pin=?");
		pst.setLong(1, account_number);
		pst.setString(2, security_pin);
		ResultSet resultSet=pst.executeQuery();
		if(resultSet.next())
